...
# Maximum time to wait for retry-after header (in seconds)
# If retry-after exceeds this, we skip retrying and return the error to the user
...
class PostForcelistRetry(Retry):
    ...
    def get_retry_after(self, response: BaseHTTPResponse) -> Optional[float]:
        ...
        instead of seconds (values > 60 are assumed to be milliseconds).
        If the final value exceeds MAX_RETRY_AFTER_SECONDS, we skip retrying
        and let the error surface to the user.

        Args:
            response: The HTTP response object.

        Returns:
            The number of seconds to wait, or None to skip retrying.
        """
        ...
        # If value > 60, assume it was sent in milliseconds by mistake
        if retry_after > 60:
            logger.debug(
                "Retry-After header value %s seems too large (likely milliseconds). "
                "Converting to seconds: %.3f",
                retry_after,
                retry_after / 1000,
            )
            retry_after = retry_after / 1000

        # If still > threshold, skip retry entirely
        if retry_after > MAX_RETRY_AFTER_SECONDS:
            logger.warning(
                "Retry-After value %.3f seconds exceeds threshold (%s seconds). "
                "Skipping retry and returning error to user.",
                retry_after,
