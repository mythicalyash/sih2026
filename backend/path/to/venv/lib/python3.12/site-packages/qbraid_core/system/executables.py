...
def get_python_executables(unique_versions: bool = True) -> dict[str, dict[str, Path]]:
    """
    .. deprecated::
        Use :func:`qbraid_core.services.environments.find_system_pythons` instead.
    ...
    warnings.warn(
        "get_python_executables is deprecated. "
        "Use qbraid_core.services.environments.find_system_pythons instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    try:
        ...
        logging.debug("Conda not available, skipping: %s", err)
        lines = []
    ...
    try:
        ...
        logger.debug("Error getting Conda Python executables: %s", err)
        conda_executables = {}
