...
def make_env_dir_name(name: str, env_id: str) -> str:
    """Generate environment directory name: ``{name_prefix}_{env_id}``.

    Takes the first 6 characters of *name*, lowercased, with non-alphanumeric
    characters replaced by underscores and trailing underscores stripped.
    Falls back to just *env_id* if the name produces no valid prefix.
    """
    prefix = re.sub(r"[^a-z0-9]", "_", name[:6].lower()).strip("_")
    return f"{prefix}_{env_id}" if prefix else env_id
...
def installed_envs_data(use_registry: bool = True) -> tuple[dict[str, Path], dict[str, str]]:
    ...
    Note:
        ...
        cloud file sync is in progress (checked via ~/.qbraid_sync_progress.json).
    """
    # Import here to avoid circular dependency
    from .registry import EnvironmentRegistryManager  # pylint: disable=import-outside-toplevel
    from .sync_progress import is_sync_in_progress  # pylint: disable=import-outside-toplevel
    ...
    if use_registry:
        try:
            ...
            # Skip sync if cloud file sync is in progress (e.g., during pod startup)
            if not is_sync_in_progress():
                registry_mgr.sync_with_filesystem()
            else:
                logger.debug("Skipping registry sync: cloud file sync in progress")
