...
export const CodePanel: React.FC<CodePanelProps> = ({ circuitIR, onApplyIR }) => {
  ...
  return (
    <div className="bg-[#fffdf9] border border-[#ded7cb] rounded-lg flex flex-col h-full shadow-sm overflow-hidden font-mono text-xs select-none">
      <div className="flex items-center justify-between px-3 py-2 border-b border-[#ded7cb] bg-[#f0ece4]">
        <div className="flex items-center gap-2">
          <select
            ...
            className="appearance-none bg-[#fffdf9] border border-[#ded7cb] text-[#211f1b] text-xs px-2 py-1 pr-6 rounded focus:outline-none focus:border-[#c96b2c] cursor-pointer"
          >
        ...
        <div className="flex items-center gap-1.5">
          {viewMode === 'ir' && (
            <button
              ...
              className="px-2 py-1 rounded bg-[#fffdf9] text-[#211f1b] hover:bg-[#eee9df] border border-[#ded7cb] text-[11px] flex items-center gap-1 cursor-pointer"
            >
              <Edit3 className="w-3 h-3 text-[#c96b2c]" />
          ...
          >
            {copied ? (
              <>
                ...
                <span className="text-[#4f806d] font-sans">Copied</span>
              </>
            ) : (
              <>
                <Copy className="w-3 h-3 text-[#746e64]" />
      ...
      {parseError && (
        <div className="p-2 bg-[#fce8e6] border border-[#ea4335] text-[#c5221f] text-[11px]">
          {parseError}
        </div>
      )}
      ...
      {isEditing ? (
        <div className="p-2 flex flex-col gap-2 flex-1">
          <textarea
            ...
            className="w-full h-[220px] bg-[#fcfbf9] border border-[#c96b2c] rounded p-2 text-xs font-mono text-[#211f1b] focus:outline-none resize-none"
          />
          <button
            onClick={handleApply}
            className="w-full py-1.5 rounded bg-[#c96b2c] hover:bg-[#b55e24] text-white font-sans text-xs font-semibold flex items-center justify-center gap-1 cursor-pointer shadow-sm"
          >
        ...
        <div className="flex-1 overflow-auto p-3 bg-[#fcfbf9] max-h-[280px]">
          <table className="w-full text-left font-mono text-xs border-collapse">
            <tbody>
              {lines.map((line, idx) => {
                ...
                return (
                  <tr key={idx} className="hover:bg-[#f0ece4]/60">
                    <td className="pr-4 py-0.5 text-[#a8a196] select-none text-right w-6 text-[11px]">
                      {idx + 1}
                    </td>
                    <td className="py-0.5 whitespace-pre">
                      {isComment ? (
                        <span className="text-[#746e64] italic">{line}</span>
                      ) : isInclude ? (
                        <span className="text-[#0f62fe] font-bold">{line}</span>
                      ) : isDecl ? (
                        <span className="text-[#137333] font-semibold">{line}</span>
                      ) : isMeasure ? (
                        <span className="text-[#b9573e] font-semibold">{line}</span>
                      ) : (
                        <span className="text-[#211f1b]">{line}</span>
                      )}
                    </td>
                  </tr>
                );
